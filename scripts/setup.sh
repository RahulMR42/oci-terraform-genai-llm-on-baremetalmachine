...
llm_port_default=8000
llm_port_openai=8001
#Execution
mkdir -p ${log_dir}
touch ${log_file}
echo "[`date`]--- Start of Script ---" |tee -a ${log_file} 2>&1
sudo sh -c "echo '########################################################################' >> /etc/motd"
sudo sh -c "echo 'welcome to host powered by ${gpu_type} powered by ${ol_version}' >> /etc/motd"
sudo sh -c "echo '########################################################################' >> /etc/motd"
sudo sh -c "echo 'loaded llm : ${llm_version}' >> /etc/motd"
...
echo "python -m vllm.entrypoints.openai.api_server --model ${model_path} --tensor-parallel-size ${parallel_gpu_count} --port ${llm_port_openai}" >>bash_openai.sh
